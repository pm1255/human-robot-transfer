...
from v3.common import state7,apply_target,dump,sha,SIZE
...
def run(a):
    ...
    if ckpt:
        model=GripperPolicy().cuda().eval();model.load_state_dict(ckpt['model'],strict=True)
    ...
    for seed in seeds:
        ...
        try:
            ...
            if not ckpt:
                ...
                rec['valid_scene']=True;env.set_instruction(instruction='Pick up the empty cup and place it on the coaster.')
