...
sha256sum v2/policy.py v2/train.py > "$ROOT/training/code_sha256.txt"
# Two independent seeds; fixed stage lengths and identical robot minibatch stream within each seed.
for seed in 17 29; do
 timeout 5400 python3 -u -m v2.train --robot "$ROOT/data/robot" --human "$ROOT/annotations/human" --out "$ROOT/training/seed$seed" --seed "$seed" --pre-steps 2000 --fine-steps 4000 | tee "$ROOT/training/seed$seed.log"
done
