...
def run(a):
 ...
 for row in rows:
  ...
  folder=ROOT/'inputs'/row['id'];output=ROOT/'results'/a.model/row['id']/f'seed{a.seed}';output.mkdir(parents=True,exist_ok=True)
 ...
 (ROOT/('GENERATION_'+a.model+'_DONE.json')).write_text(json.dumps({'model':model_name,'seed':a.seed}))
if __name__=='__main__':
 p=argparse.ArgumentParser();p.add_argument('--model',choices=['1.3B','14B'],default='1.3B');p.add_argument('--size',type=int,default=512);p.add_argument('--steps',type=int,default=40);p.add_argument('--seed',type=int,default=2026);p.add_argument('--only');run(p.parse_args())
