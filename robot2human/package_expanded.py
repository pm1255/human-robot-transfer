...
def run(model):
 ...
 for meta in sorted((ROOT/'results'/model).glob('*/seed*/result.json')):
  ...
  for row in labels['samples']:
   identity='|'.join([row['uid'],data['model'],str(data['seed']),data['source_sha256'],data.get('mask_sha256') or hashlib.sha256((source/'masks.npz').read_bytes()).hexdigest(),data.get('variant','baseline')])
